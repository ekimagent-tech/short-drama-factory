...
export const db = {
  ...
  createUser: (user: Omit<User, 'createdAt' | 'updatedAt'>) => {
    ...
    if (useSqlite && db) {
      sqliteDb.prepare(`INSERT INTO users (id, name, email, password, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)`)
  ...
  createProject: (project: Omit<Project, 'createdAt' | 'updatedAt'>) => {
    ...
    if (useSqlite && db) {
      sqliteDb.prepare(`INSERT INTO projects (id, userId, name, description, status, theme, outline, script, settings, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  getProjectsByUserId: (userId: string) => {
    if (useSqlite && db) {
      return sqliteDb.prepare('SELECT * FROM projects WHERE userId = ? ORDER BY createdAt DESC').all(userId);
    }
  ...
  updateProject: (id: string, updates: Partial<Project>) => {
    ...
    if (useSqlite && db) {
      ...
      sqliteDb.prepare(`UPDATE projects SET ${fields}, updatedAt = ? WHERE id = ?`).run(...values, now, id);
      return sqliteDb.prepare('SELECT * FROM projects WHERE id = ?').get(id);
    }
  ...
  createScene: (scene: Omit<Scene, 'createdAt' | 'updatedAt'>) => {
    ...
    if (useSqlite && db) {
      sqliteDb.prepare(`INSERT INTO scenes (id, projectId, orderNum, duration, description, characterDescription, cameraMovement, dialogue, backgroundMusic, emotionTag, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  getScenesByProjectId: (projectId: string) => {
    if (useSqlite && db) {
      return sqliteDb.prepare('SELECT * FROM scenes WHERE projectId = ? ORDER BY orderNum').all(projectId);
    }
  ...
  updateScene: (id: string, updates: Partial<Scene>) => {
    ...
    if (useSqlite && db) {
      ...
      sqliteDb.prepare(`UPDATE scenes SET ${fields}, updatedAt = ? WHERE id = ?`).run(...values, now, id);
      return sqliteDb.prepare('SELECT * FROM scenes WHERE id = ?').get(id);
    }
  ...
  deleteScene: (id: string) => {
    if (useSqlite && db) {
      sqliteDb.prepare('DELETE FROM scenes WHERE id = ?').run(id);
    }
  ...
  createCharacter: (character: Omit<Character, 'createdAt' | 'updatedAt'>) => {
    ...
    if (useSqlite && db) {
      sqliteDb.prepare(`INSERT INTO characters (id, userId, name, description, imageUrl, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  getCharactersByUserId: (userId: string) => {
    if (useSqlite && db) {
      return sqliteDb.prepare('SELECT * FROM characters WHERE userId = ? ORDER BY createdAt DESC').all(userId);
    }
  ...
  updateCharacter: (id: string, updates: Partial<Character>) => {
    ...
    if (useSqlite && db) {
      const fields = Object.keys(updates).map(k => `${k} = ?`).join(', ');
      const values = Object.values(updates);
      sqliteDb.prepare(`UPDATE characters SET ${fields}, updatedAt = ? WHERE id = ?`).run(...values, now, id);
      return sqliteDb.prepare('SELECT * FROM characters WHERE id = ?').get(id);
    }
  ...
  deleteCharacter: (id: string) => {
    if (useSqlite && db) {
      sqliteDb.prepare('DELETE FROM characters WHERE id = ?').run(id);
    }
